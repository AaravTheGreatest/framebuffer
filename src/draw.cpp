...
int edge(const Vertex& a, const Vertex& b, float x, float y) {
...
int drawTriangle(Framebuffer& fb, const Vertex& a, const Vertex& b, Vertex c, bool filled) {
    ...
    for (int y = yMin; y <= yMax; y++) {
      for (int x = xMin; x <= xMax; x++){
        int p1 = edge(a, b, x, y),
            p2 = edge(b, c, x, y),
            p3 = edge(c, a, x, y);
        if ((p1 >= 0 && p2 >= 0 && p3 >= 0) || (p1 <= 0 && p2 <= 0 && p3 <= 0)) fb.setPixel(x, y, a.color);
      }
    }
  }
  return 0;
